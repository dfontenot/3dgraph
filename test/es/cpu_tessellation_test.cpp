...
TEST(CPUTessellation, InspectLattice) {
    ...
    EXPECT_EQ(2 * any_tessellation_amount * any_tessellation_amount, lattice.size());
    ...
    // top left point
    auto x_idx = 2 * (any_tessellation_amount - 1);
    EXPECT_FLOAT_EQ(-0.5f, lattice[x_idx]);
    EXPECT_FLOAT_EQ(0.5f, lattice[x_idx + 1]);

    // bottom right point
    x_idx = 2 * (any_tessellation_amount) * (any_tessellation_amount - 1);
    EXPECT_FLOAT_EQ(0.5f, lattice[x_idx]);
    EXPECT_FLOAT_EQ(-0.5f, lattice[x_idx + 1]);
