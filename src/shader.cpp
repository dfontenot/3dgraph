...
Shader::Shader(const char* source_fn, GLenum shader_type) : shader_type(shader_type), shader_handle(glCreateShader(shader_type)) {
    ...
    if (log_bytes_to_allocate > 1) {
        ...
        if (compiled == GL_TRUE) {
            cerr << "encountered error when compiling " << shader_type_to_string(shader_type) << " shader: " << compilation_err_str.get() << endl;
        }
        else {
